...
class AnalyzeBusyHoursJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        $user = $this->user;
        Log::info(
            "Starting queued busy hours analysis for user {$user->id} ({$user->name})",
        );
        ...
        try {
            ...
            if ($transactions->isEmpty()) {
                Log::warning(
                    "No transactions found for user {$user->id} during queued busy hours analysis.",
                );
                return;
            }
            ...
            if ($response->successful()) {
                ...
                Log::info(
                    "Successfully ran and saved queued busy hours analysis for user {$user->id}",
                );
            } else {
                AiRun::create([
                    "user_id" => $user->id,
                ...
                Log::error(
                    "API error during queued busy hours analysis for user {$user->id}: " .
                        $response->body(),
                );
            }
        } catch (\Exception $e) {
            Log::error(
                "Exception in queued busy hours analysis for user {$user->id}: " .
                    $e->getMessage(),
            );
            AiRun::create([
                "user_id" => $user->id,
